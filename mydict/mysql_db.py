...
'''
    数据库操作模块：
    将数据库操作封装一个类，将dict_server需要的数据库操作功能
    分别写成方法，在dict_server中实例化对象，需要什么方法直接调用
'''
...
class Database:
    ...
    def register(self,name,passwd):
        ...
        r = self.cur.fetchone()  # 查找不到返回None,查找到了返回元组
        if r:
            return False  # 用户存在
        # # 密码加密存储处理
        hash = hashlib.md5((name+SALT).encode())  # 生成对象,并加盐处理
        ...
        try:
            ...
            self.db.rollback()  # 退回到commit 执行之前的数据库状态
            return False
